...
def action_start():
    ...
    if result == "already_running":
        ...
        run_dialog_nostdout(
            "--title", tr("error"), "--msgbox", f"{tr('start_failed')}\n\n{result}", "10", "60"
        )
...
def action_restart():
    ...
    if result == "started":
        ...
        run_dialog_nostdout(
            "--title", tr("error"), "--msgbox", f"{tr('restart_failed')}\n\n{result}", "10", "60"
        )
...
def action_models():
    ...
    try:
        ...
        if result.returncode != 0:
            run_dialog_nostdout(
                "--title", tr("error"), "--msgbox", tr("fetch_models_err"), "8", "50"
            )
            return
        ...
        if not models_info:
            run_dialog_nostdout(
                "--title", tr("ollama_manager"), "--msgbox", tr("no_models"), "8", "50"
            )
            return

        run_dialog_nostdout(
            "--title", tr("ollama_manager"), "--msgbox", "\n".join(models_info), "15", "60"
        )
    except Exception:
...
def action_pull_model():
    ...
    run_dialog_nostdout(
        "--title", tr("download_title"), "--infobox", tr("download_msg", model_name), "10", "60"
    )

    try:
        result = subprocess.run(
            ["ollama", "pull", model_name], capture_output=True, text=True, timeout=3600
        )

        if result.returncode == 0:
            run_dialog_nostdout(
                "--title", tr("success"), "--msgbox", tr("download_success", model_name), "8", "60"
            )
        else:
...
def action_remove_model():
    ...
    try:
        ...
        if run_dialog_nostdout(
            "--title", tr("confirm"), "--yesno", tr("remove_confirm", selected_model), "8", "60"
        ):
            subprocess.run(["ollama", "rm", selected_model], capture_output=True)
            run_dialog_nostdout(
                "--title", tr("menu_remove"), "--msgbox", tr("model_removed"), "8", "50"
            )
    except Exception:
        run_dialog_nostdout("--title", tr("error"), "--msgbox", tr("fetch_models_err"), "8", "50")
...
def action_autocorrect_status():
    missing_deps = [
        cmd
        for cmd in ["wl-copy", "wl-paste", "wtype", "curl", "notify-send"]
